...
trait TestKitBase {
   ...
   */
  def expectTerminated(target: ActorRef, existenceConfirmed: Option[Boolean] = None, max: Duration = Duration.Undefined): Terminated =
    expectMsgPF(max, "Terminated " + target) {
      case t @ Terminated(`target`) if existenceConfirmed.forall(_ == t.existenceConfirmed) ⇒ t
    }
